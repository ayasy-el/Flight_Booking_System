...
const logger = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.File({
      filename: path.join("logs", "notification-service.log"),
      maxsize: 5242880, // 5MB
      maxFiles: 5,
    }),
    // Separate file for error logs
    new winston.transports.File({
      filename: path.join("logs", "notification-service-error.log"),
      level: "error",
      maxsize: 5242880, // 5MB
      maxFiles: 5,
    }),
  ...
  exceptionHandlers: [
    ...
    new winston.transports.File({
      filename: path.join("logs", "notification-service-exceptions.log"),
      maxsize: 5242880, // 5MB
      maxFiles: 5,
    }),
  ],
